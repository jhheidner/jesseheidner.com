...
def _history_to_messages(history: list) -> list[dict]:
    """Convert Gradio history (tuples or message dicts) to OpenAI message format."""
    messages = []
    for item in history:
...
class Me:
    ...
    def chat(self, message: str, history: list) -> str:
        if not os.getenv("OPENAI_API_KEY"):
            return (
                "The chatbot is not configured yet. Set OPENAI_API_KEY in the "
                ".env file (local) or Hugging Face Space secrets."
            )
